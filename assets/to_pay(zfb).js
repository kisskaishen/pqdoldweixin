...
new Vue({
	...
	methods: {
		...
		get_data: function(){
			var self_ = this;
			var url = sign.signUrl('https://z.pinquduo.cn/api_2_0_2/api/goods/getOrder'+location.search+'&ajax_get=1');
			$.ajax({
				type:'POST',
				url: url,//获取数据
				dataType:'jsonp',
		...
		refresh_coupon:function(){
			var self_ = this;
			var url = sign.signUrl('https://z.pinquduo.cn/api_2_0_2/api/goods/getOrder'+location.search.replace('&num='+buy_count,'&num='+self_.buy_count)+'&ajax_get=1');
			$.ajax({
				type:'POST',
				url: url,//获取数据
				dataType:'jsonp',
		...
		get_address: function(){
			...
			$.ajax({
				type:'POST',
				url:'https://z.pinquduo.cn/api_2_0_2/api/goods/getUserAddressList?' + qs,//获取数据
				dataType:'jsonp',
				jsonp: 'jsoncallback',
				async:false,
				success:function(data){
		...
		get_coupon: function(){
			...
			$.ajax({
				type:'POST',
				url:'https://z.pinquduo.cn/api_2_0_2/api/goods/getCoupon?' + qs,//获取数据
				dataType:'jsonp',
				jsonp: 'jsoncallback',
				async:false,
				success:function(data){
		...
		pay: function(){
			...
			if(self_.data.goods.prom==0&&prom_type==1){
				//免单拼开团
				var payScriptUrl='https://z.pinquduo.cn/api_2_0_2/api/goods/getBuy'+location.search.replace('&num='+buy_count,'&num='+self_.buy_count)+'&address_id='+address_id+'&prom='+self_.prom_count+'&free='+self_.free_count+'&openid='+openid+'&code='+self_.pay_oauth+'&ajax_get=1';
			}else{
				var payScriptUrl='https://z.pinquduo.cn/api_2_0_2/api/goods/getBuy'+location.search.replace('&num='+buy_count,'&num='+self_.buy_count)+'&address_id='+address_id+'&openid='+openid+'&code='+self_.pay_oauth+'&ajax_get=1';
			};
			...
			payScriptUrl = sign.signUrl(payScriptUrl);
		...
		paySelect:function (event) {
            ...
            console.log(222);
			console.log($('.payItem'));
            $('.payItem i').toggleClass('hidden')
            if(event=='qpay'){
                self_.pay_oauth='qpay';
                console.log(self_.pay_oauth);
            }else{
                self_.pay_oauth='alipay'
                console.log(self_.pay_oauth);
            }
