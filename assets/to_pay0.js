...
$.ajax({
    ...
    success: function(data) {
		...
		var share_config = {
		    ...
		    shareUrl: share_url,
		    link: share_url, // 分享链接
		    imgUrl: 'https://z.pinquduo.cn/api_2_0_2/Template/pc/pinquduo/Static/images/pqdlogo.jpg', // 分享图标
...
setShareInfo({
    ...
    pic: 'https://z.pinquduo.cn/api_2_0_2/Template/pc/pinquduo/Static/images/pqdlogo.jpg',
    url: share_url
});
...
new Vue({
	...
	data: function(){
		return {
			...
			//跳转到收货地址列表
			adr_group_route: 'adr_add.html'+location.search+'&pay_adr_route=topay',
			//是否拼团/参团，单买 价格判断
	...
	methods: {
		...
		get_data: function(){
			...
			$.ajax({
				type:'POST',
				url:'https://z.pinquduo.cn/api_2_0_2/api/goods/getOrder'+location.search+'&ajax_get=1',//获取数据
				dataType:'jsonp',
				jsonp: 'jsoncallback',
				async:false,
				success:function(data){
					...
					self_.coupon_price = data.result.coupon!=null?data.result.coupon.money:0;
					self_.coupon_id = data.result.coupon!=null?data.result.coupon.id:0;
					self_.coupon_list_id = data.result.coupon!=null?data.result.coupon.coupon_list_id:0;
					self_.price = self_.is_prom?data.result.goods.prom_price:data.result.goods.shop_price;
					self_.coupon_price = ((self_.coupon_price*100)/100).toFixed(2);
					//改变购买数量 获取优惠券
		...
		refresh_coupon:function(){
			...
			$.ajax({
				type:'POST',
				url:'https://z.pinquduo.cn/api_2_0_2/api/goods/getOrder'+location.search.replace('&num='+buy_count,'&num='+self_.buy_count)+'&ajax_get=1',//获取数据
				dataType:'jsonp',
				jsonp: 'jsoncallback',
				async:false,
				success:function(data){
		...
		get_address: function(){
			...
			$.ajax({
				type:'POST',
				url:'https://z.pinquduo.cn/api_2_0_2/api/goods/getUserAddressList?user_id='+user_id+'&ajax_get=1',//获取数据
				dataType:'jsonp',
				jsonp: 'jsoncallback',
				async:false,
				success:function(data){
					...
					if(self_.data.user){
						for(var i=0,len = self_.adr_data.length;i < len;i++){
							if(urlSearch('address_id')==self_.adr_data[i].address_id){
								self_.data.user = self_.adr_data[i];
								break;
							}
						};
					};
		...
		get_coupon: function(){
			...
			$.ajax({
				type:'POST',
				url:'https://z.pinquduo.cn/api_2_0_2/api/goods/getCoupon?user_id='+user_id+'&store_id='+store_id+'&goods_id='+goods_id+'&num='+self_.buy_count+'&type='+coupon_type+'&ajax_get=1',//获取数据
				dataType:'jsonp',
				jsonp: 'jsoncallback',
				async:false,
				success:function(data){
					...
					if(self_.coupon_data!=null&&self_.coupon_data.length>0&&self_.coupon_list_id){
						self_.has_coupon = true;
						for(var i=0;i<self_.coupon_data.length;i++){
							//self_.coupon_data[i].use_start_time=formatDate(new Date(data.result.items[i].use_start_time*1000));
							self_.coupon_data[i].use_end_time=formatDate(new Date(data.result.items[i].use_end_time*1000));
						};
					}else{
						self_.has_coupon = false;
					};
				},
				error: function(xhr,type){
				    console.log('Ajax error!');
		...
		select_coupon: function(name,event){
			...
			self_.coupon_price = self_.coupon_data[index].money;
			self_.coupon_id = self_.coupon_data[index].id;
		...
		pay: function(){
			...
			if(self_.data.goods.prom==0&&prom_type==1){
				//免单拼开团
				var payScriptUrl='https://pqd/api/goods/getBuy'+location.search.replace('&num='+buy_count,'&num='+self_.buy_count)+'&address_id='+address_id+'&prom='+self_.prom_count+'&free='+self_.free_count+'&openid='+openid+'&code='+self_.pay_oauth+'&ajax_get=1';
			}else{
				//var payScriptUrl='https://pqd/api/goods/getBuy'+location.search.replace('&num='+buy_count,'&num='+self_.buy_count)+'&address_id='+address_id+'&openid='+openid+'&code='+self_.pay_oauth+'&ajax_get=1';
				var payScriptUrl='https://pqd/api/goods/getBuy'+location.search.replace('&num='+buy_count,'&num='+self_.buy_count)+'&address_id='+address_id+'&openid='+openid+'&code=alipay&ajax_get=1';
			};
			//使用优惠券
