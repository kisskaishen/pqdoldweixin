...
new Vue({
    ...
    mounted: function(){
        ...
        this.tab = location.href.indexOf('nav')>-1?true:false
    ...
    methods: {
        ...
        getHomeBanner:function () {
            let that = this
            get('ad/positions',{}).then(function (data) {
                data = data.data
                console.log(data);
                that.sideImg = data.result.listSide
            })
        },
        //上拉加载
        up_load: function(){
        ...
        showNative(val) {
            if (this.userInfoType == 'i') {
                showGoodsDetails(val.toString()) 
            } else {
                webview.showGoodsDetails(val.toString())
            }
        }
