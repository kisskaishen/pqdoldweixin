...
new Vue({
	...
	methods: {
		...
		get_Address: function(edit_id){
			...
			$.ajax({
				type:'POST',
				url:'https://z.pinquduo.cn/api_3_0_1/goods/getUserAddressList/user_id/5838?version=2.0.0&user_id='+user_id+'&ajax_get=1',//获取数据
				dataType:'jsonp',
				jsonp: 'jsoncallback',
				async:true,
				success:function(data){
...
$('#save-btn').click(function(){
    ...
    if(!is_save){
    	$.ajax({
			type:'POST',
			url:'https://z.pinquduo.cn/api_3_0_1/goods/addEidtAddress'+'?version=2.0.0&user_id='+user_id+'&address_id='+address_id+'&consignee='+name+'&default='+is_default+'&address_base='+address_base+'&address='+street+'&mobile='+mobile+'&type='+(is_pay_route?2:urlSearch('type'))+'&ajax_get=1',//获取数据
			dataType:'jsonp',
			jsonp: 'jsoncallback',
			async:true,
			success:function(data){
