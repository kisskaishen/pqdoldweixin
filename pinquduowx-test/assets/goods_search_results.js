...
new Vue({
	...
	methods: {
		...
		get_results: function(){
			var self_ = this;
			//搜索关键字
			self_.key_words = urlSearch('key_words')||'';
		    ...
		    $.ajax({
				type:'POST',
				url:'https://testapi.pinquduo.cn/api_3_0_1/goods/getsearch'+'?version=2.0.0&key='+self_.key_words+'&page='+self_.page+'&pagesize='+self_.pagesize+'&ajax_get=1',//获取数据
				dataType:'jsonp',
				jsonp: 'jsoncallback',
				async:false,
				success:function(data){
			        ...
			        if(data.result.items.length == 0){
			        	...
			        	self_.search_hint = '没有找到相关商品';
			        };
			        if(data.result.total>0){
			        	self_.search_hint = '已为你找到'+data.result.total+'条'+'相关商品';
