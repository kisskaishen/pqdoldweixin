	...
	mounted: function(){
        ...
        if(isWeiXin()||isQQ()){
			...
			if(!cookie.get('openid')){
				...
				$.ajax({
					type:'POST',
					url:'https://testapi.pinquduo.cn/api_3_0_1/user/thirdLogin?version=2.0.0&ajax_get=1'+'&nickname='+cookie.get('user_name')+'&head_pic='+cookie.get('head_pic')+'&oauth='+oauth+'&openid='+cookie.get('openid'),
					dataType:'jsonp',
					jsonp: 'jsoncallback',
					async:true,
					success:function(data){
